...
def build_refresh_js () -> str:
    ...
    return """// GENERATED by Scripts/Chronicle - the freshness poller (v0.2.6.0).
(function () {
  if (location.protocol !== "http:" && location.protocol !== "https:") return;
  var s = document.currentScript;
  var root = s && s.src ? s.src.replace(/assets\\/refresh\\.js.*$/, "") : "";
  var last = null;
  function tick () {
    if (document.visibilityState !== "visible") return;
    fetch(root + "index.html", {method: "HEAD", cache: "no-store"})
      .then(function (r) {
        if (!r.ok) return;                      // RV25a
        var lm = r.headers.get("last-modified");
        if (!lm) return;                        // RV25a
        if (last === null) { last = lm; return; }
        if (lm !== last) location.reload();
      })
      .catch(function () {});                   // network blip: skip
  }
...
def build_mkdocs_yml (nav: list, mermaid_js: str, bootswatch_css: str,
                      site_url: str) -> str:
    """The generated config. T1 use_directory_urls false; T3 retired
    WITH `mkdocs serve` (v0.2.6.0 CFT-2 - refresh.js is the freshness
    path now); T6 (RV16): the fence MUST be mermaid2.fence_mermaid -
    ...
    lines = [
        ...
        f"  - {yq('assets/refresh.js')}",  # v0.2.6.0: freshness poller
        "plugins:",
        "  - search",
        "  - mermaid2:",
