...
FastAPI server (single process)
  ...
  · SQLite (events · tasks · commits)
        ▼
REST + WebSocket → React UI (**KATLAB City — the living workspace: an isometric city view where each repo is a district, buildings grow from file churn, live events rain in, commits fire fireworks, in-progress tasks stand as cranes, and Kat walks the streets under a day/night sky** · status bar + branch chips + session pulse + CLEAN ✓ celebration + live capture combo + Kat the workspace pet (mood mirrors repo truth) · task sidebar · changes-by-task · diff viewer · file story · manual-pick queue · Overview dashboard: KPIs, the live plan board (active missions, in-progress spotlight, next-up), week-over-week momentum strip, charts, year calendar + streak + isometric skyline city + activity snake, day lanes + 30-second activity replay + radial day clock, weekly wrapped, punch card, change coupling list + arc constellation, daily goal rings, repo identity card, codebase heat treemap, trophy case + personal records (break-day celebrations), task→commit map · ambient focus mode (wall display + rings + pet) · session dots + session timeline · effort estimates · discipline guard · attention bell + rhythm nudges + OS alerts · Ctrl+K palette · daily digest export · one-click 7/30-day HTML reports (self-contained, shareable, print-to-PDF) · live per-repo stats badges at `/badge/<repo>.svg` (local README previews) · live status favicon (the tab tells the truth) · one-click City SVG snapshots · **open-in-editor links** (every file story jumps straight into VS Code) · **system health panel** (`/api/health` self-audit: watcher liveness, hook line presence, capture freshness + ingest-lag hover, DB/event-log stats) · view-transition nav · history + commit graph); installable as a PWA whose taskbar icon carries the live uncommitted count (badge on the installed app, network-only service worker — nothing cached)
```
